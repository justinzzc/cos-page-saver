...
const editorList = document.querySelector("#editorProfileList");
...
function openEditor(profile) {
  ...
  form.reset();
  fill(profile || { name: "新建 COS", objectPrefix: "clips", multipartConcurrency: 20 });
  render();
  showMessage("");
}
...
function profileItem(profile) {
  ...
  details.append(heading, badge, meta);
  details.addEventListener("click", () => openEditor(profile));
  const actions = document.createElement("div"); actions.className = "profile-actions";
  actions.append(
    actionButton("编辑", () => openEditor(profile)),
    actionButton("复制", async () => { const copy = await saveProfile({ ...profile, id: "", name: uniqueCopyName(profile) }); openEditor(copy); showMessage(`已复制为“${copy.name}”`); }),
    ...
    actionButton("删除", async () => { if (!confirm(`确定删除“${profile.name}”吗？`)) return; await removeProfile(profile.id); state = await getProfileState(); if (selectedId === profile.id) { const next = state.profiles[0]; if (next) fill(next); else { editing = false; selectedId = ""; } } await refresh(); showMessage("配置已删除"); })
  );
  item.append(details, actions);
  return item;
}
function render() {
  profilesView.hidden = editing;
  editorView.hidden = !editing;
  list.replaceChildren(...state.profiles.map(profileItem));
  editorList.replaceChildren(...(editing ? state.profiles.map(profileItem) : []));
...
backButton.addEventListener("click", () => { editing = false; selectedId = ""; render(); showMessage(""); });
...
deleteButton.addEventListener("click", async () => { if (!selectedId || !confirm("确定删除当前配置吗？")) return; await removeProfile(selectedId); state = await getProfileState(); const next = state.profiles[0]; if (next) { selectedId = next.id; fill(next); } else { editing = false; selectedId = ""; } await refresh(); showMessage("配置已删除"); });
